...
1. Каков IP-адрес вашего хоста? Каков IP-адрес хоста назначения?
   - 192.168.0.130, 150.100.8.18
   - ![alt text](https://github.com/arnyyyyy/networks-course/blob/master/lab09/images/t11.png)
2. Почему ICMP-пакет не обладает номерами исходного и конечного портов?
   - Так как ICMP находится на сетевом уровне, в то время как порты находятся на транспортном.
3. Рассмотрите один из ping-запросов, отправленных вашим хостом. Каковы ICMP-тип и кодовый
   номер этого пакета? Какие еще поля есть в этом ICMP-пакете? Сколько байт приходится на поля 
   контрольной суммы, порядкового номера и идентификатора?
   - Type 8 (Echo (ping) request)
   - Checksum, Identifier (BE) + Identifier (LE), Sequence Number (BE) + Sequence Number (LE), Data, Timestamp (они не вошли в скрин, но они есть)
   - По 2 байта на поля контрольной суммы, порядкового номера и идентификатора
   - ![alt text](https://github.com/arnyyyyy/networks-course/blob/master/lab09/images/t12.png)
5. Рассмотрите соответствующий ping-пакет, полученный в ответ на предыдущий. 
   ...
   - Checksum, Identifier (BE) + Identifier (LE), Sequence Number (BE) + Sequence Number (LE), Data, Timestamp (они не вошли в скрин, но они есть)
   - По 2 байта на поля контрольной суммы, порядкового номера и идентификатора
   - ![alt text](https://github.com/arnyyyyy/networks-course/blob/master/lab09/images/t13.png)
...
1. Рассмотрите ICMP-пакет с эхо-запросом на вашем скриншоте. Отличается ли он от ICMP-пакетов
   ...
   - Да, появилось поле Time To Live
   - ![alt text](https://github.com/arnyyyyy/networks-course/blob/master/lab09/images/t21.png)
3. Рассмотрите на вашем скриншоте ICMP-пакет с сообщением об ошибке. В нем больше полей,
   чем в ICMP-пакете с эхо-запросом. Какая информация содержится в этих дополнительных полях?
   - Данные IPv4 исходного запроса, поле unused
   - ![alt text](https://github.com/arnyyyyy/networks-course/blob/master/lab09/images/t22.png)
4. Рассмотрите три последних ICMP-пакета, полученных исходным хостом. Чем эти пакеты
   ...
   - Нет полей из третьего пункта
   - Другой ICMP-тип (Type: 11 (Time-to-live exceeded) vs Type: 0 (Echo (ping) reply))
   - Пакеты доходят до цели, и она отвечает обычным ICMP Echo Reply
   - ![alt text](https://github.com/arnyyyyy/networks-course/blob/master/lab09/images/t23.png)
6. Есть ли такой канал, задержка в котором существенно превышает среднее значение? Можете
   ...
   - Пришлось использовать сервис для определения местоположения хоста по IP-адресу, оба маршрутизатора находятся в Дублине
   - ![alt text](https://github.com/arnyyyyy/networks-course/blob/master/lab09/images/traceroute.png)
...
#### Демонстрация работы
![alt text](https://github.com/arnyyyyy/networks-course/blob/master/lab09/images/taskA.png)
...
#### Демонстрация работы
![alt text](https://github.com/arnyyyyy/networks-course/blob/master/lab09/images/taskB.png)
